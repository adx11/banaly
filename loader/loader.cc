...
#define BFD_ERRMSG (bfd_errmsg(bfd_get_error()))
...
static int load_symbols_bfd(bfd *bfd_h, Binary *bin)
{
    ...
    if (n < 0) {
        ...
        auto nsyms = bfd_canonicalize_symtab(bfd_h, bfd_symtab);
        if (nsyms < 0) {
            fprintf(stderr, "failed to read symtab (%s)\n",
                    BFD_ERRMSG);
            goto fail;
...
static int load_dynsyms_bfd(bfd *bfd_h, Binary *bin)
{
    int ret;
    asymbol **bfd_dynsym = NULL;
    Symbol *sym;

    auto n = bfd_get_dynamic_symtab_upper_bound(bfd_h);
    if (n < 0) {
        fprintf(stderr, "failed to read symtab (%s)\n",
                BFD_ERRMSG);
        goto fail;
        ...
        auto nsyms = bfd_canonicalize_dynamic_symtab(bfd_h, bfd_dynsym);
        if (nsyms < 0) {
...
static int load_binary_bfd(std::string &fname, Binary *bin, Binary::BinaryType type)
{
    ...
    /* Symbol hadling is best-effort only */
    load_symbols_bfd(bfd_h, bin);
    load_dynsyms_bfd(bfd_h, bin);
